...
## D-024 · 2026-09-23 · Deterministic duplicate-question detection
- **Context**: a second live user test (a follow-up, obesity + disc hernia) exposed a much more serious version of D-023's item 3: MedGemma repeated near-identical questions up to 3 times in a single ~10-question session (e.g. "¿Ha notado alguna diferencia en su peso desde la última consulta?" three times), and in one case appeared to resubmit a stale answer against a different question — the patient's own words were "esto es incompetente." D-023's prompt-only fix ("never repeat a question") was not sufficient; the model doesn't follow it reliably.
- **Decision**: added a deterministic code-level guard, `raup/questionnaire/dedup.py` — `is_duplicate_question` normalizes (lowercase, strip punctuation) and compares a candidate question against every previously asked question in the session via `difflib.SequenceMatcher`, flagging anything ≥0.75 similarity, not just exact repeats. `raup/questionnaire/engine.py` checks every generated question against this before ever showing it to the patient: on a duplicate, it retries (up to 2 extra attempts) with an explicit reminder naming the repeated question; if still duplicating after all attempts, it ends the interview gracefully rather than ever displaying a repeated question. Also tightened the prompt's wording (`raup/questionnaire/prompts.py`): a follow-up must be a *new, more specific* question, never the same question again or a re-check of something already answered clearly.
- **Reasoning**: the same throughline as D-009/D-016/D-020/D-022 — a text instruction is a request, not a guarantee; anywhere a concrete failure mode is observed, the fix is a deterministic check in code, not a stronger-worded prompt alone (the prompt wording was still tightened, but as the first layer, not the only one).
- **Verified live**: a scripted 5-turn follow-up simulation against the real endpoint, using a similar scenario to the failing session (obesity + disc hernia, canned patient answers keyed to question topic), produced **zero duplicate or near-duplicate questions** across the run, ending naturally via DONE after covering general status, physical activity, pain level, diet, and daily-activity difficulty — a sensible spread, not a loop.
- **Not yet independently re-verified**: with the actual reporting user, on the real deployed app, on a first-visit session (only follow-up was scripted-tested), or with a human typing genuinely varied answers rather than the topic-keyed canned responses used in the simulation script.
...
- Whether D-024's dedup similarity threshold (0.75) needs tuning once more real, human-typed sessions are observed
...
- Whether D-023's "don't leave a patient-raised lead unexplored" prompt change actually reduces premature endings with a real human typing varied answers (D-024's simulation used canned answers and ended naturally, but that's a different check than "did it skip something a real patient would have wanted explored")
